...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new AppError("There was an error sending the email. Try it again later!"),
      500
    );
  }
});
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  // 5) if new password different current password
  if (!!(await user.correctPassword(req.body.password, user.password))) {
    return next(
      new AppError(
        "Your current password and new password must not be the same ",
        401
      )
    );
  }
